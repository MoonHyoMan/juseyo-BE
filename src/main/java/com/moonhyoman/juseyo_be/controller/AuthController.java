...
public class AuthController {
    ...
    @Autowired
    public AuthController(AuthService authService,KakaoLoginService kakaoLoginService, JwtGenerator jwtGenerator) {
...
//        log.info("user: {}", user.getId());
    ...
    @Parameters({
            ...
            @Parameter(name = "parentId", description = "필수값X<br>" +
                    "근데 자식 회원가입이면 넣어야 함." +
    ...
    @Operation(summary = "kakao 로그인",
            description = "kakao 로그인 API<br>" +
                    "카카오 인가코드를 보내고<br>" +
                    "아이디가 있다면 토큰 발급<br>" +
                    "없으면 400에러와 로그인 실패 return")
    @Parameters({
